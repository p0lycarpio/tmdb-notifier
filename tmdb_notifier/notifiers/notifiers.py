...
class Notifiers:
    def __init__(self, configuration: Configuration, http: HTTPSession) -> None:
        ...
        if self.apprise:
            ...
            self.notifier = Webhook(self.webhook, self.webhook_type, http)
        else:
            raise ValueError(
                "No notifier configured. Please set APPRISE_URL or WEBHOOK_URL"
            )

    async def send(self, movie: Movie, services: str):
        body = self.create_message(movie, services)
        if asyncio.iscoroutinefunction(self.notifier._send):
            await self.notifier._send(movie, body)
        else:
            self.notifier._send(movie, body)
