...
@pytest.mark.asyncio
async def test_get_watchlist(tmdb):
...
@pytest.mark.asyncio
async def test_get_credits(tmdb):
    movie = await tmdb.get_movie(0, json.loads(_load_fixture("movie.json")))
    credits = await tmdb.get_credits(0, json.loads(_load_fixture("credits.json")))
...
@pytest.mark.asyncio
async def test_get_providers(tmdb):
